const { User, Job_Info, Job_Application, Company_Details } = require("../models/index");
...
exports.getAllUsers = async (req, res) => {
  try {
    ...
    if (!users)
      return res.status(400).json({ message: "Not found users in DB!" });
  ...
  try {
    ...
    const usersWithJobDetails = await User.findAndCountAll({
      ...
      include: {
        ...
        attributes: {
          exclude: ["id", "admin_id", "updated_at", "created_at"],
        },
        required: false,
      },
      attributes: {
    ...
    if (!usersWithJobDetails)
      return res.json({ message: "No users available" });
    console.log(usersWithJobDetails);
    usersWithJobDetails.totalPages = Math.ceil(
      usersWithJobDetails.count / size
    );
    usersWithJobDetails.currentPage = size;
...
exports.getJobById = async (req, res) => {
  try {
    let { page, size } = req.query;
    const usersWithJobDetails = await User.findOne({
      where: {
        is_admin: false,
        id: req.params.jobId
      },
